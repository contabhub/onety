...
// Consulta o Inter, atualiza "boletos" e grava "boleto_status_historico" quando houver mudança
...
router.post("/cobranca", async (req, res) => {
  const { conta_corrente, seuNumero, valorNominal, dataVencimento, numDiasAgenda, pagador, formasRecebimento, mensagem, company_id, contrato_id } = req.body;
  
  try {
    // Buscar conta Inter configurada para a empresa
    let companyId = company_id || 1; // Usar company_id da requisição ou padrão
    ...
    // Garante que seuNumero tenha no máximo 15 caracteres
    let seuNumeroFinal = seuNumero;
    if (typeof seuNumeroFinal === 'string' && seuNumeroFinal.length > 15) {
      seuNumeroFinal = seuNumeroFinal.slice(0, 15);
    }
    // Monta o objeto do boleto conforme a documentação do Inter
    const boletoData = {
      seuNumero: seuNumeroFinal,
      valorNominal,
      dataVencimento,
      numDiasAgenda,
      pagador,
    ...
    const [boletoResult] = await pool.query(
      `INSERT INTO boletos 
        (link_boleto, codigo_barras, data_emissao, data_vencimento, status, seu_numero, valor_nominal, pagador_nome, pagador_cpf_cnpj, pagador_email, codigo_solicitacao, inter_account_id, contrato_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        linkBoleto,
        codigoBarras,
        ...
        dataVencimentoFinal,
        status,
        seuNumeroFinal, // garantir que está usando o seuNumero final
        valorNominal,
        pagador?.nome || null,
...
router.get('/consulta/:codigoSolicitacao', async (req, res) => {
  ...
  try {
    ...
    if (!boleto || !boleto.inter_account_id) {
      return res.status(404).json({ error: 'Boleto não encontrado ou sem inter_account_id configurado' });
    }
...
router.get('/teste-boleto/:codigoSolicitacao', async (req, res) => {
  ...
  try {
    ...
    const AutomacaoRecorrencia = require('../services/automacaoRecorrencia');
...
router.get('/teste-simples/:codigoSolicitacao', async (req, res) => {
  ...
  try {
    // 1) Buscar o boleto para obter o inter_account_id
    const [[boleto]] = await pool.query(
      'SELECT id, inter_account_id FROM boletos WHERE codigo_solicitacao = ? LIMIT 1',
      [codigoSolicitacao]
    );
    ...
    if (!boleto.inter_account_id) {
      return res.status(400).json({ error: "Boleto não tem inter_account_id configurado" });
    }
    ...
    if (!contaInter) {
      return res.status(400).json({ error: `Conta Inter ${boleto.inter_account_id} não encontrada` });
    }
    ...
    // 4) Salvar token no cache com inter_account_id
    await pool.query(
...
router.get('/boletos/por-venda/:vendaId', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      valorNominal: boleto.valor_nominal,
...
// ✅ Buscar boleto pelo seu_numero (contém info da venda)
router.get('/boletos/por-seu-numero/:seuNumero', async (req, res) => {
  const { seuNumero } = req.params;
  try {
    console.log(`🔍 Buscando boleto pelo seu_numero: ${seuNumero}`);
    ...
    const [rows] = await pool.query(
      `SELECT 
        ...
        created_at
      FROM boletos 
      WHERE seu_numero LIKE ? 
      ORDER BY created_at DESC 
      LIMIT 1`,
      [`%${seuNumero}%`]
    );
    
    console.log(`📊 Boletos encontrados para seu_numero ${seuNumero}:`, rows);
    
    if (rows.length === 0) {
      return res.status(404).json({ 
        error: 'Nenhum boleto encontrado para esse seu_numero.',
        seuNumero: seuNumero,
        debug: 'Verifique se o seu_numero está correto'
      });
    }
    ...
    console.error("Erro ao buscar boleto por seu_numero:", error);
    return res.status(500).json({ error: "Erro interno ao buscar boleto." });
  }
});

// ✅ Corrigir venda_id NULL baseado no seu_numero
router.patch('/boletos/corrigir-venda-id/:boletoId', async (req, res) => {
  ...
  try {
    ...
    const [boletoRows] = await pool.query(
      'SELECT id, seu_numero, venda_id FROM boletos WHERE id = ?',
      [boletoId]
    );
    ...
    // Extrair venda_id do seu_numero (formato: VENDA_XXX_XXXXX)
    const vendaMatch = boleto.seu_numero?.match(/VENDA_(\d+)_/);
    if (!vendaMatch) {
      return res.status(400).json({ 
        error: 'Não foi possível extrair venda_id do seu_numero',
        seuNumero: boleto.seu_numero
      });
    }
    ...
    const [result] = await pool.query(
      'UPDATE boletos SET venda_id = ?, updated_at = NOW() WHERE id = ?',
      [vendaId, boletoId]
    );
    ...
    const [updatedRows] = await pool.query(
      'SELECT id, venda_id, seu_numero, codigo_solicitacao FROM boletos WHERE id = ?',
      [boletoId]
    );
...
router.post('/boletos/corrigir-todos-venda-id', async (req, res) => {
  try {
    ...
    // Buscar todos os boletos com venda_id NULL mas com seu_numero de venda
    const [boletosParaCorrigir] = await pool.query(
      `SELECT id, seu_numero, venda_id 
       FROM boletos 
       WHERE venda_id IS NULL 
         AND seu_numero LIKE 'VENDA_%_%'`
    );
...
router.get('/boletos/codigo-por-venda/:vendaId', async (req, res) => {
  ...
  try {
    ...
    // Busca por venda_id OU pelo seu_numero (backup)
    const [rows] = await pool.query(
      `SELECT codigo_solicitacao, id, seu_numero, venda_id
       FROM boletos 
       WHERE venda_id = ? 
          OR seu_numero LIKE ?
       ORDER BY created_at DESC 
    ...
    return res.json({
      ...
      vendaId: vendaId,
      boletoId: boleto.id,
      valor_nominal: boleto.valor_nominal || venda?.valor_venda,
...
const cronBoletos = require('../services/cronBoletos');
...
router.get('/cron/boletos-pendentes', async (req, res) => {
  try {
    const [boletos] = await pool.query(`
      SELECT id, codigo_solicitacao, status, created_at, valor_nominal
        FROM boletos
       WHERE codigo_solicitacao IS NOT NULL
         AND (status IS NULL OR UPPER(status) NOT IN ('PAGO','RECEBIDO','LIQUIDADO','CANCELADO','BAIXADO','EXPIRADO'))
       ORDER BY created_at DESC
...
// 🔧 ROTA TEMPORÁRIA: Atualizar boletos existentes com inter_account_id
router.post('/fix-existing-boletos', async (req, res) => {
  try {
    ...
    const [[colunaExiste]] = await pool.query(`
      ...
      AND COLUMN_NAME = 'inter_account_id'
    `);
    ...
    if (colunaExiste.existe === 0) {
      ...
      await pool.query(`
        ALTER TABLE boletos 
        ADD COLUMN inter_account_id INT NULL AFTER venda_id
      `);
      console.log("✅ Coluna inter_account_id adicionada");
    }
    ...
    // 3. Contar boletos sem inter_account_id
    const [[boletosSemInter]] = await pool.query(`
      SELECT COUNT(*) as total 
      FROM boletos 
      WHERE inter_account_id IS NULL 
      AND codigo_solicitacao IS NOT NULL
    `);
    
    if (boletosSemInter.total === 0) {
      return res.json({
        message: "Todos os boletos já têm inter_account_id configurado",
        total: 0
      });
    }
    ...
    const [resultado] = await pool.query(`
      UPDATE boletos 
      SET inter_account_id = ? 
      WHERE inter_account_id IS NULL 
      AND codigo_solicitacao IS NOT NULL
    `, [contaInter.id]);
    
    console.log(`✅ ${resultado.affectedRows} boletos atualizados com inter_account_id = ${contaInter.id}`);
...
router.post('/fix-boleto/:codigoSolicitacao', async (req, res) => {
  try {
    ...
    const [[boleto]] = await pool.query(`
      SELECT id, codigo_solicitacao, inter_account_id
      FROM boletos 
      WHERE codigo_solicitacao = ?
    `, [codigoSolicitacao]);
    ...
    // 2. Verificar contas Inter disponíveis
    const [contasInter] = await pool.query(`
      SELECT id, nome, ambiente, company_id, apelido
      FROM inter_accounts 
      ORDER BY id
    `);
    ...
    console.log(`✅ Boleto ${codigoSolicitacao} atualizado com inter_account_id = ${contaInter.id}`);
    ...
    res.json({
      ...
      boleto: {
        id: boleto.id,
        codigo_solicitacao: boleto.codigo_solicitacao,
        inter_account_id_anterior: boleto.inter_account_id,
        inter_account_id_novo: contaInter.id
      },
      conta_utilizada: {
        id: contaInter.id,
        nome: contaInter.nome,
...
router.post('/fix-boleto-credenciais/:codigoSolicitacao', async (req, res) => {
  try {
    ...
    // 1. Verificar se o boleto existe
    const [[boleto]] = await pool.query(`
      SELECT b.id, b.codigo_solicitacao, b.inter_account_id, 
             ia.apelido, ia.client_id, ia.ambiente
      FROM boletos b
      LEFT JOIN inter_accounts ia ON b.inter_account_id = ia.id
    ...
    res.json({
      ...
      boleto: {
        ...
        codigo_solicitacao: boleto.codigo_solicitacao,
        inter_account_id_anterior: boleto.inter_account_id,
        inter_account_id_novo: contaValida.id
      },
      conta_utilizada: {
        id: contaValida.id,
...
router.post('/test-criar-transacao/:codigoSolicitacao', async (req, res) => {
  try {
    ...
    // 1) Buscar o boleto
    const [[boleto]] = await pool.query(`
      SELECT id, status, valor_nominal, seu_numero, codigo_solicitacao
      FROM boletos 
      WHERE codigo_solicitacao = ?
    `, [codigoSolicitacao]);
    ...
    const campos = {
      statusBruto: 'PAGO',
      valorRecebido: boleto.valor_nominal,
    ...
    res.json({
      ...
      boleto: {
        ...
        valor_nominal: boleto.valor_nominal
      },
      transacao: {
        id: transacaoId,
        tipo: 'RECEITA',
...
router.post('/atualizar-venda-recebido/:codigoSolicitacao', async (req, res) => {
  try {
    ...
    // 1) Buscar o boleto
    const [[boleto]] = await pool.query(`
      SELECT id, status, valor_nominal, valor_recebido, data_pagamento, venda_id
      FROM boletos
      WHERE codigo_solicitacao = ?
    `, [codigoSolicitacao]);
    ...
    const campos = {
      ...
      statusConsolidado: 'PAGO',
      valorRecebido: boleto.valor_recebido || boleto.valor_nominal,
...
router.post('/test-criar-para-pago/:codigoSolicitacao', async (req, res) => {
  try {
    ...
    const [[boleto]] = await pool.query(`
      SELECT id, status, valor_nominal, seu_numero, codigo_solicitacao, pagador_cpf_cnpj, company_id
      FROM boletos
      WHERE codigo_solicitacao = ?
    `, [codigoSolicitacao]);
    ...
    if (vendas.length === 0) {
      ...
      if (vendaId) {
        const [[vendaCriada]] = await pool.query(`
          SELECT id, categoria_id, sub_categoria_id, valor_venda, created_at 
...
router.get('/verificar-status/:codigoSolicitacao', async (req, res) => {
  try {
    ...
    // Buscar boleto
    const [[boleto]] = await pool.query(`
      SELECT id, status, valor_nominal, seu_numero, codigo_solicitacao, pagador_cpf_cnpj, company_id, contrato_id
      FROM boletos
      WHERE codigo_solicitacao = ?
    `, [codigoSolicitacao]);
    ...
    const [transacoes] = await pool.query(`
      SELECT id, descricao, valor, created_at FROM transacoes WHERE boleto_id = ?
    `, [boleto.id]);

    // Verificar vendas
    const [vendas] = await pool.query(`
      SELECT id, tipo_venda, situacao, valor_venda, categoria_id, sub_categoria_id, created_at FROM vendas WHERE boleto_id = ?
    `, [boleto.id]);
