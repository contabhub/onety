...
router.post("/", verifyToken, async (req, res) => {
  try {
    const { arquivoBase64, conta_id, company_id } = req.body;
    ...
    for (const t of transacoes) {
      ...
      const status = "recebido"; // Status fixo para transações importadas

      await pool.query(`
        INSERT INTO transacoes 
        (conta_id, company_id, tipo, valor, descricao, data_transacao, origem, situacao, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
      `, [
        conta_id,
        company_id,
