  ...
  try {
    ...
    if (moduloId) {
      const [rows] = await pool.query(
        "SELECT SQL_CALC_FOUND_ROWS * FROM grupo_conteudo WHERE modulo_id = ? ORDER BY id DESC LIMIT ? OFFSET ?",
    ...
    const [rows] = await pool.query(
      "SELECT SQL_CALC_FOUND_ROWS * FROM grupo_conteudo ORDER BY id DESC LIMIT ? OFFSET ?",
      [limit, offset]
    );
    const [countRows] = await pool.query("SELECT FOUND_ROWS() as total");
...
router.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await pool.query("SELECT * FROM grupo_conteudo WHERE id = ?", [id]);
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const [result] = await conn.query(
      `INSERT INTO grupo_conteudo (nome, descricao, modulo_id) VALUES (?, ?, ?)`,
      [nome, descricao || null, modulo_id]
    );

    await conn.commit();

    const [created] = await pool.query("SELECT * FROM grupo_conteudo WHERE id = ?", [result.insertId]);
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    const sql = `UPDATE grupo_conteudo SET ${fields.join(", ")} WHERE id = ?`;
    await pool.query(sql, [...values, id]);

    const [updated] = await pool.query("SELECT * FROM grupo_conteudo WHERE id = ?", [id]);
...
router.patch("/:id/logo", upload.single("logo"), async (req, res) => {
  ...
  try {
    ...
    conn = await pool.getConnection();
    await conn.beginTransaction();
    await conn.query("UPDATE grupo_conteudo SET logo_url = ? WHERE id = ?", [finalUrl, id]);
    await conn.commit();
...
router.delete("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [existing] = await pool.query("SELECT id FROM grupo_conteudo WHERE id = ?", [id]);
    if (existing.length === 0) return res.status(404).json({ error: "Conteúdo não encontrado." });

    await pool.query("DELETE FROM grupo_conteudo WHERE id = ?", [id]);
    res.status(204).send();
  } catch (error) {
    console.error(error);
