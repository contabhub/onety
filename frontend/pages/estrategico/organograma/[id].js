...
export default function DepartmentDetails() {
  ...
  const departmentId = params?.departmentId || params?.id || null;
  const companyIdParam = params?.companyId || null;
  const companyId = companyIdParam || getEmpresaId();
  ...
  const mapTaskStatus = (status) => {
    ...
    switch (normalized) {
      ...
      case 'concluída após prazo':
      case 'concluida apos prazo':
      case 'concluída após meta':
      case 'concluída após meta/prazo':
      case 'concluida apos meta':
  ...
  const normalizeTasks = (tasks = [], membersList = []) => {
    return tasks.map((task) => {
      const status = mapTaskStatus(task.status);
      const responsibleMember = membersList.find((member) => member.id === task.responsavel_id);
      const assignee = responsibleMember
        ? {
            id: responsibleMember.id,
            full_name: responsibleMember.full_name,
            avatar_url: responsibleMember.avatar_url ?? null,
            email: responsibleMember.email ?? null,
          }
        : {
            id: task.responsavel_id,
            full_name: task.responsavel || 'Sem responsável',
            avatar_url: null,
            email: null,
          };
      ...
      return {
        id: task.id,
        title: task.assunto || task.nome || 'Tarefa',
        description: task.descricao || '',
        status,
        rawStatus: task.status,
        start_date: task.data_acao || task.data_inicio || null,
        due_date: task.data_prazo || null,
        end_date: task.data_prazo || null,
        completed_at: task.data_conclusao || null,
        assignee,
        assignees: [],
        atividades: task.atividades || [],
        cliente_nome: task.cliente_nome || null,
        cliente_status: task.status_cliente || null,
        departamento_id: task.departamento_id || null,
        departamento_nome: task.departamento || task.departamento_nome || null,
        responsavel_id: task.responsavel_id || null,
      };
    });
  };

  const normalizeMonthlyGoals = (monthlyGoals = []) =>
    (monthlyGoals || []).map((mg) => ({
      ...mg,
      start_date: mg.start_date || mg.data_inicio || null,
      end_date: mg.end_date || mg.data_fim || null,
      value_goal: mg.value_goal ?? mg.valor_alvo ?? 0,
      value_achieved: mg.value_achieved ?? mg.valor_alcancado ?? 0,
      status: mg.status,
    }));
  ...
  const normalizeDepartmentGoal = (goal) => {
    ...
    return {
      ...goal,
      title: goal.title || goal.nome || goal.name || 'Meta departamental',
      ...
      calculation_type: goal.calculation_type || goal.calculo_tipo || null,
      indicator_type: goal.indicator_type || goal.indicador_tipo || null,
      progress_type: goal.progress_type || goal.progresso_tipo || null,
      monthlyGoals: normalizeMonthlyGoals(goal.monthlyGoals),
    };
  };
    ...
    const loadDepartment = async () => {
      try {
        if (!companyId || !departmentId || !user) {
          console.warn('[DepartmentDetails] Skip loadDepartment: companyId:', companyId, 'departmentId:', departmentId, 'user:', user?.id);
          return;
        }

        // 🔹 Carrega detalhes do departamento
        console.log('[DepartmentDetails] Chamando departamento:', departmentId, companyId);
        const deptDetails = await apiFetch(`/estrategico/departamentos/${departmentId}?companyId=${companyId}`);
        console.log('[DepartmentDetails] Detalhes departamento:', deptDetails);
        setDepartment(deptDetails);

        // 🔹 Carrega membros
        const deptMembers = await apiFetch(`/gestao/departamentos/${departmentId}/usuarios`);
        console.log('[DepartmentDetails] Membros carregados:', deptMembers?.length);
        setMembers(deptMembers || []);

        // 🔹 Carrega tarefas do departamento (sempre para calcular estatísticas de performance)
        let deptTasks = [];
        try {
          const deptTasksResponse = await apiFetch(`/gestao/tarefas/empresa/${companyId}/departamentos/${departmentId}`);
          deptTasks = normalizeTasks(deptTasksResponse || [], deptMembers || []);
          setDepartmentTasks(deptTasks);
        } catch (error) {
          setDepartmentTasks([]);
        }

        // 🔹 Carrega metas do departamento
        const deptGoalsResponse = await apiFetch(`/estrategico/metas-departamentais?departmentId=${departmentId}&companyId=${companyId}`);
        const deptGoalsData =
          Array.isArray(deptGoalsResponse?.data)
            ? deptGoalsResponse.data
            : Array.isArray(deptGoalsResponse)
              ? deptGoalsResponse
              : [];
        setDepartmentGoals(deptGoalsData.map(normalizeDepartmentGoal));

        // As estatísticas serão calculadas em um useEffect separado após carregar tarefas e membros
  ...
  const loadGoals = async () => {
    try {
      ...
      if (!Array.isArray(deptGoals)) {
        console.error('❌ [DepartmentDetails] deptGoals não é um array:', deptGoalsResponse);
        setGoals([]);
        return;
      }
      ...
      const goalsWithMonths = await Promise.all(
        deptGoals.map(async (goal) => {
          const monthlyGoalsResponse = await apiFetch(`/estrategico/metas-departamentais/${goal.id}/months`);
          const normalizedMonthlyGoals = normalizeMonthlyGoals(monthlyGoalsResponse);

          // Se um mês específico estiver selecionado, use apenas os dados desse mês
          if (selectedMonth) {
            const monthGoal = normalizedMonthlyGoals.find((m) => {
          ...
          const current_value = normalizedMonthlyGoals.reduce((acc, m) => acc + (Number(m.value_achieved) || 0), 0);
          const target_value = normalizedMonthlyGoals.reduce((acc, m) => acc + (Number(m.value_goal) || 0), 0);
  ...
  }, [companyId, departmentId, selectedTrimestre, selectedYear, selectedMonth, user]);
  ...
  useEffect(() => {
    const loadChildren = async () => {
      ...
      try {
        const children = await apiFetch(`/estrategico/departamentos?companyId=${companyId}`);
  ...
  const handleToggleTasks = async (employeeId) => {
    ...
    if (!employeeTasks[employeeId]) {
      try {
        const employeeTasksResponse = await apiFetch(`/gestao/tarefas/empresa/${companyId}/usuarios/${employeeId}`);
        const tasks = normalizeTasks(employeeTasksResponse || [], members);
  ...
  useEffect(() => {
    async function fetchEmployeePerformances() {
      ...
      await Promise.all(members.map(async (emp) => {
        const employeeTasksResponse = await apiFetch(`/gestao/tarefas/empresa/${companyId}/usuarios/${emp.id}`);
        const tasks = normalizeTasks(employeeTasksResponse || [], members);
                  ...
                  <>
                    {members.length > 0 ? (
                      <div className={styles.teamMembersGrid}>
                        {members.map(emp => {
                          const memberName = (emp?.full_name || emp?.nome || emp?.name || emp?.email || 'Sem nome').toString();
                          const memberInitial = memberName.trim().charAt(0).toUpperCase() || 'U';

                          return (
                            <div key={emp.id ?? memberName} className={styles.memberCard}>
