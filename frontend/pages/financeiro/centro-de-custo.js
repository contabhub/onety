...
import styles from "../../styles/financeiro/centro-de-custo.module.css";
...
export default function CentrosCustoPage() {
  ...
  const getSortIcon = (field) => {
    ...
    return sortDirection === "asc" ? (
      <ChevronUp className={styles.centroDeCustoTableSortIcon} />
    ) : (
      <ChevronDown className={styles.centroDeCustoTableSortIcon} />
    );
  };

  const getStatusBadge = (situacao) => {
    return situacao === "Ativo" ? (
      <Badge className={styles.centroDeCustoBadge + ' ' + styles.centroDeCustoBadgeAtivo}>Ativo</Badge>
    ) : (
      <Badge className={styles.centroDeCustoBadge + ' ' + styles.centroDeCustoBadgeInativo}>Inativo</Badge>
    );
  };
  ...
  return (
    <>
      <PrincipalSidebar />
      <div className={styles.centroDeCustoPage}>
      {/* Header */}
      <div className={styles.centroDeCustoHeader}>
        <div>
          <h1 className={styles.centroDeCustoHeaderTitle}>Centros de custo</h1>
          <p className={styles.centroDeCustoHeaderSubtitle}>Gerencie seus centros de custo</p>
        </div>
        <div className={styles.centroDeCustoHeaderActions}>
          <Button
            size="sm"
            className={styles.centroDeCustoNovoBtn}
            onClick={() => setIsNovoModalOpen(true)}
          >
            <Plus className={styles.centroDeCustoActionIcon} />
      ...
      {/* Search */}
      <div className={styles.centroDeCustoSearchContainer}>
        <div className={styles.centroDeCustoSearchLabel}>Pesquisar</div>
        <div className={styles.centroDeCustoSearchWrapper}>
          <Search className={styles.centroDeCustoSearchIcon} />
          <Input
            placeholder="Pesquisar"
            className={styles.centroDeCustoSearchInput}
            value={searchTerm}
      ...
      {/* Stats Cards */}
      <div className={styles.centroDeCustoStatsGrid}>
        <Card
          className={`${styles.centroDeCustoStatCard} ${statusFilter === "Ativo" ? styles.centroDeCustoStatCardActiveAtivo : ''}`}
          onClick={handleFilterAtivos}
        >
          {statusFilter === "Ativo" && (
            <div className={styles.centroDeCustoStatCardIndicator + ' ' + styles.centroDeCustoStatCardIndicatorAtivo}></div>
          )}
          <CardContent className={styles.centroDeCustoStatCardContent}>
            <p className={styles.centroDeCustoStatCardLabel}>Ativos</p>
            <p className={styles.centroDeCustoStatCardValue + ' ' + styles.centroDeCustoStatCardValueAtivo}>
              {stats.ativos}
            </p>
          </CardContent>
        </Card>
        ...
        <Card
          className={`${styles.centroDeCustoStatCard} ${statusFilter === "Inativo" ? styles.centroDeCustoStatCardActiveInativo : ''}`}
          onClick={handleFilterInativos}
        >
          {statusFilter === "Inativo" && (
            <div className={styles.centroDeCustoStatCardIndicator + ' ' + styles.centroDeCustoStatCardIndicatorInativo}></div>
          )}
          <CardContent className={styles.centroDeCustoStatCardContent}>
            <p className={styles.centroDeCustoStatCardLabel}>Inativos</p>
            <p className={styles.centroDeCustoStatCardValue + ' ' + styles.centroDeCustoStatCardValueInativo}>
              {stats.inativos}
            </p>
          </CardContent>
        </Card>
        ...
        <Card
          className={`${styles.centroDeCustoStatCard} ${statusFilter === "Todos" ? styles.centroDeCustoStatCardActiveTodos : ''}`}
          onClick={handleFilterTodos}
        >
          {statusFilter === "Todos" && (
            <div className={styles.centroDeCustoStatCardIndicator + ' ' + styles.centroDeCustoStatCardIndicatorTodos}></div>
          )}
          <CardContent className={styles.centroDeCustoStatCardContent}>
            <p className={styles.centroDeCustoStatCardLabel}>Todos</p>
            <p className={styles.centroDeCustoStatCardValue + ' ' + styles.centroDeCustoStatCardValueTodos}>{stats.todos}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Filtro Ativo */}
      {statusFilter !== "Todos" && (
        <div className={styles.centroDeCustoFilterContainer}>
          <Badge variant="outline" className={styles.centroDeCustoFilterBadge}>
            Filtro: {statusFilter}
          </Badge>
          <Button
            variant="ghost"
            size="sm"
            onClick={handleFilterTodos}
            className={styles.centroDeCustoClearFilterBtn}
          >
            Limpar filtro
          </Button>
        </div>
      ...
      {/* Table */}
      <Card className={styles.centroDeCustoTableCard}>
        <CardContent className={styles.centroDeCustoTableContent}>
          {loading ? (
            <div className={styles.centroDeCustoLoading}>
              <div className={styles.centroDeCustoLoadingSpinner}></div>
              <p className={styles.centroDeCustoLoadingText}>
                Carregando centros de custo...
              </p>
            </div>
          ) : (
            <div className={styles.centroDeCustoTableContainer}>
              <table className={styles.centroDeCustoTable}>
                <thead className={styles.centroDeCustoTableHeader}>
                  <tr className={styles.centroDeCustoTableRow}>
                    <th className={styles.centroDeCustoTableHeaderCell}>
                      <button
                        onClick={() => handleSort("codigo")}
                        className={styles.centroDeCustoTableHeaderBtn}
                      >
                        Código
                        {getSortIcon("codigo")}
                      </button>
                ...
                <tbody>
                  {sortedCentros.map((centro) => (
                    <tr key={centro.id} className={styles.centroDeCustoTableRow}>
                      <td className={styles.centroDeCustoTableCell}>{centro.codigo || "-"}</td>
                      <td className={styles.centroDeCustoTableCell + ' ' + styles.centroDeCustoTableCellPrimary}>{centro.nome}</td>
                      <td className={styles.centroDeCustoTableCell}>{getStatusBadge(centro.situacao)}</td>
                      <td className={styles.centroDeCustoTableCell}>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              variant="outline"
                              size="sm"
                              className={styles.centroDeCustoActionBtn}
                            >
                              <MoreVertical className={styles.centroDeCustoDropdownIcon} />
                            </Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end" className={styles.centroDeCustoDropdown}>
                            <DropdownMenuItem
                              onClick={() => {
                                setCentroSelecionado(centro);
                                setIsEditModalOpen(true);
                              }}
                              className={styles.centroDeCustoDropdownItem}
                            >
                              <Edit className={styles.centroDeCustoDropdownIcon} />
                              Editar
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              ...
                              className={styles.centroDeCustoDropdownItem + ' ' + styles.centroDeCustoDropdownItemDanger}
                            >
                              <Trash2 className={styles.centroDeCustoDropdownIcon} />
                              Excluir
                            </DropdownMenuItem>
                            <DropdownMenuItem
                            ...
                            >
                              {centro.situacao === "Ativo" ? (
                                <>
                                  <XCircle className={styles.centroDeCustoDropdownIcon} />
                                  Inativar
                                </>
                              ) : (
                                <>
                                  <CheckCircle className={styles.centroDeCustoDropdownIcon} />
                                  Ativar
                                </>
                              )}
                            </DropdownMenuItem>
          ...
          {!loading && sortedCentros.length === 0 && (
            <div className={styles.centroDeCustoEmptyState}>
              <p className={styles.centroDeCustoEmptyStateText}>
                {statusFilter !== "Todos"
      ...
      <Dialog open={deleteModal.isOpen} onOpenChange={() => setDeleteModal({ isOpen: false })}>
        <DialogContent className={styles.centroDeCustoModal}>
          <DialogHeader className={styles.centroDeCustoModalHeader}>
            <DialogTitle className={styles.centroDeCustoModalTitle}>Excluir centro de custo</DialogTitle>
          </DialogHeader>

          <div className={styles.centroDeCustoModalBody}>
            <p className={styles.centroDeCustoModalText}>
              Deseja excluir o centro de custo <strong className={styles.centroDeCustoModalTextStrong}>{deleteModal.centro?.nome}</strong>?
            </p>
          </div>

          <div className={styles.centroDeCustoModalFooter}>
            <Button 
              variant="outline" 
              onClick={() => setDeleteModal({ isOpen: false })}
              className={styles.centroDeCustoModalButtonSecondary}
            >
              Cancelar
            </Button>
            <Button
              onClick={handleDeleteCentro}
              className={styles.centroDeCustoModalButtonPrimary}
            >
