// pages/dashboard/obrigacoes/[id]/atividades.tsx
"use client";
...
type StatusObrigacao = keyof typeof coresPorStatus;


function obterStatusObrigacao(vencimento: Date): StatusObrigacao {
    const hoje = new Date();
...
function somarDias(date: Date, qtd: number, tipo: "Dias úteis" | "Dias corridos") {
...
function subtrairDias(date: Date, qtd: number, tipo: "Dias úteis" | "Dias corridos") {
...
function renderComentarioAmigavel(comentario: string) {
...
export default function AtividadesObrigacao() {
    ...
    const [obrigacao, setObrigacao] = useState<any>(null);
    ...
    const [atividadeCancelar, setAtividadeCancelar] = useState<any>(null);
    ...
    const [pdfLayoutSelecionado, setPdfLayoutSelecionado] = useState<any>(null);
    const [pdfLayouts, setPdfLayouts] = useState<any[]>([]);
    ...
    const [pdfValidationResult, setPdfValidationResult] = useState<any>(null);
    const [emailAssunto, setEmailAssunto] = useState("");
    const [emailCorpo, setEmailCorpo] = useState("");
    const [emailModalAberto, setEmailModalAberto] = useState(false);
    const [modalUploadAberto, setModalUploadAberto] = useState(false);
    const [anexosExpandidos, setAnexosExpandidos] = useState<{[key: number]: boolean}>({});
    ...
    const [obrigacaoAnterior, setObrigacaoAnterior] = useState<any>(null);
    const [obrigacaoProxima, setObrigacaoProxima] = useState<any>(null);
    const [loadingNavegacao, setLoadingNavegacao] = useState(false);

    // ✅ Função para buscar obrigações adjacentes (anterior e próxima)
    const buscarObrigacoesAdjacentes = async (obrigacaoAtual: any) => {
        ...
        try {
            ...
            if (obrigacoesAdjacentes && obrigacoesAdjacentes.length > 0) {
                ...
                const posicaoAtual = obrigacoesAdjacentes.findIndex(
                    (ob: any) => ob.id === obrigacaoAtual.id
                );
    ...
    const navegarParaObrigacao = (obrigacaoDestino: any) => {
    ...
    const carregarDados = async () => {
        ...
        try {
            ...
            const [{ data: obrigacao }, { data: atividadesCliente }, { data: comentarios }, { data: responsaveis }] = await Promise.all([
                api.get(`/api/obrigacoes/cliente-obrigacao/${id}`, { headers }),
                api.get(`/api/obrigacoes/atividades-cliente/${id}`, { headers }),
                api.get(`/api/obrigacoes/${id}/comentarios`, { headers }),
                api.get(`/api/obrigacoes/obrigacoes-clientes/${id}/responsaveis`, { headers }),
            ]);
            ...
            setAtividades(atividadesCliente);
    ...
    function formatarHoraBrasilia(data: string | Date | undefined | null): string {
        if (!data) return "--:--";
        const d = typeof data === "string" ? parseMysqlDatetime(data) : data;
        if (!d || isNaN(d.getTime())) return "--:--";
    ...
    function formatarCnpjCpf(cnpjCpf: string | null | undefined): string {
        if (!cnpjCpf) return "";
    ...
    const concluirAtividade = async (atividadeId: number, tipo: string) => {
        const token = sessionStorage.getItem("token");
        if (!token) return;
        try {
            await api.patch(`/api/obrigacoes/atividade/${atividadeId}/concluir`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
            ...
            // ✅ Recarregar dados e verificar se todas as atividades foram concluídas
            const { atividades: atividadesAtualizadas, obrigacao: obrigacaoAtualizada } = await carregarDados();

            // ✅ Verificar se todas as atividades foram finalizadas e a obrigação ainda não foi concluída
            if (atividadesAtualizadas && atividadesAtualizadas.length > 0 && obrigacaoAtualizada) {
                const todasFinalizadas = atividadesAtualizadas.every((a: any) => a.concluida === 1 || a.cancelada === 1);
                const obrigacaoNaoConcluida = !obrigacaoAtualizada.dataBaixa && obrigacaoAtualizada.status !== "concluida" && obrigacaoAtualizada.status !== "cancelada";

                if (todasFinalizadas && obrigacaoNaoConcluida) {
                    console.log("🎉 Todas as atividades finalizadas! Concluindo obrigação automaticamente...");
                    toast.info("Todas as atividades finalizadas! Concluindo obrigação automaticamente...");
                    
                    // Concluir a obrigação automaticamente
                    await api.patch(`/api/obrigacoes/${id}/concluir`, {}, {
                        headers: { Authorization: `Bearer ${token}` },
                    });
                    
                    toast.success("Obrigação concluída automaticamente!");
                    
                    // Recarregar dados novamente para refletir a conclusão da obrigação
                    await carregarDados();
                }
            }
    ...
    const cancelarAtividade = async (atividadeId: number) => {
        const token = sessionStorage.getItem("token");
        if (!token) return;
        try {
            await api.patch(`/api/obrigacoes/atividade/${atividadeId}/cancelar`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });

            toast.success("Atividade cancelada com sucesso.");
            
            // ✅ Recarregar dados e verificar se todas as atividades foram finalizadas
            const { atividades: atividadesAtualizadas, obrigacao: obrigacaoAtualizada } = await carregarDados();
    ...
    const todasAtividadesFinalizadas = atividades.every((a) => a.concluida === 1 || a.cancelada === 1);
    ...
    const atividadesPendentes = atividades.filter((a) => a.concluida !== 1 && a.cancelada !== 1).length;
    ...
    const validarPdfLayout = async (atividade: any) => {
        const token = sessionStorage.getItem("token");
    ...
    const [buscandoMatches, setBuscandoMatches] = useState<number | null>(null);
    const [anexosEcontadorExpandidos, setAnexosEcontadorExpandidos] = useState<{[key: number]: boolean}>({});

    const buscarMatchesEcontador = async (atividade: any) => {
        const token = sessionStorage.getItem("token");
        if (!token || !atividade) return;
            ...
            {!obrigacaoCancelada && emailModalAberto && (
                ...
                onSend={async ({ para, cc, co, assunto, corpo, anexo }) => {
                    ...
                    try {
                        ...
                        let nomeUsuario = "Titan App", emailUsuario = "";
                        if (user) {
                          try {
                            const obj = JSON.parse(user);
                            nomeUsuario = obj.nome || "Titan App";
                            emailUsuario = obj.email || "";
                            console.log("🔍 [FRONTEND DEBUG] Email extraído do sessionStorage:", emailUsuario);
                          } catch (error) {
